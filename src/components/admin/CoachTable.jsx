...
export default function CoachTable() {
    ...
    async function fetchCoaches() {
        try {
            const { data, error } = await supabase.from('profiles').select('*').eq('role', 'coach').order('full_name', { ascending: true })
                                        ...
                                        <td className="py-4">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] uppercase tracking-wider font-bold text-purple-400/80 mt-1 flex items-center gap-1">
                                                    <span className="w-1 h-1 rounded-full bg-purple-400"></span> Coach
                                                </span>
                                            </div>
                                        </td>
                                        <td className="py-4">
