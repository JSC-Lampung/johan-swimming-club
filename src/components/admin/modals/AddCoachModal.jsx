...
export default function AddCoachModal({ isOpen, onClose, onSuccess }) {
    const [loading, setLoading] = useState(false)
    const [formData, setFormData] = useState({
        name: '', email: '', phone: '', birth_date: '', gender: '', password: '', status: 'active', program: '', image_url: ''
    })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (userId) {
                ...
                const { error: profileError } = await supabase
                    ...
                    .update({
                        ...
                        position_title: `Pelatih ${programLabel}`
                    })
                    .eq('id', userId)
            ...
            alert(`Pelatih ${formData.name} berhasil didaftarkan!`)
            onSuccess()
            onClose()
            setFormData({ name: '', email: '', phone: '', birth_date: '', gender: '', password: '', status: 'active', program: '', image_url: '' })
        } catch (err) {
            alert('Gagal menambah pelatih: ' + err.message)
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md p-8 shadow-2xl animate-fadeIn overflow-y-auto max-h-[90vh] custom-scrollbar">
                <h3 className="text-2xl font-bold text-white mb-6">Tambah Pelatih Baru</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-slate-400 text-sm mb-2">Program Melatih</label>
                        <select required
                            value={formData.program} onChange={e => setFormData({ ...formData, program: e.target.value })}
                            className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500">
                            <option value="">Pilih Program Melatih</option>
                    ...
                    <div className="pt-4 flex items-center gap-3">
                        <button type="submit" disabled={loading}
                            ...
                            {loading ? 'Memproses...' : 'Tambah Pelatih'}
                        </button>
